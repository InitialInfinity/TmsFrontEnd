...
const AddTrainingForm = () => {
  ...
  const GetAllByDepart = () => {
    console.log(departments,"Shraddha");
  ...
  const handleEmpCodeChange = (selected) => {
    ...
    console.log('Selected values for empcode handleEmpCodeChange:', selectedValues);
    setCode(selectedValues);
  
    setIsEmployeeNameDisabled(true);
    setIsDesignationDisabled(true);
    setIsDepartmentDisabled(true);
    console.log(designation, 'empcode777');
    console.log(trainingType, 'empcode777');
  
    axios
      .get(new URL(UrlData + `TrainingForm/GetByCode?td_emp_code=${selectedValues.join(',')}`))
      .then((response) => {
        console.log('Response data by code:', response.data.data);
        const temp = response.data.data;
  
        // Create lookup objects to map employee codes to their respective details
        const empCodeToDesignation = {};
        const empCodeToName = {};
        const empCodeToDepartment = {};
  
        temp.forEach((data) => {
          empCodeToDesignation[data.td_emp_code] = { value: data.td_emp_des, label: data.td_des };
          empCodeToName[data.td_emp_code] = { value: data.td_emp_name, label: data.td_emp_name };
          empCodeToDepartment[data.td_emp_code] = { value: data.td_dept, label: data.td_dept };
        });
  
        const newDesignations = selectedValues.map((code) => empCodeToDesignation[code] || {});
        const newEmpNames = selectedValues.map((code) => empCodeToName[code] || {});
        const newDepartments = selectedValues.map((code) => empCodeToDepartment[code] || {});
       ...
       if (!selectedValues.includes('select_all')) {
        ...
        console.log('Select All is chosen, skipping setDepartments.');
      }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    console.log(departments.value, 'abcd');
  ...
  const GetEmployeeByDept = () => {
    axios
      .get(new URL(UrlData + `TrainingForm/GetAllByDepart?td_dept=${departments.value}`))
      .then((response) => {
        ...
        setEmpCodeOptions([{ value: 'select_all', label: 'Select All' }, ...temp]);
        setIsDesignationDisabled(true);
  ...
  const handleTrainingNature=(e)=>{
    setTrainingType(e.target.value)
    console.log(trainingType,"training type")
  
  };
  useEffect(() => {
    console.log(designation.value, "designation use");
    console.log(trainingType, "training nature use");
    if (trainingType === "5b650378-6fd6-4e92-b182-cc48ad58e6ad") {
      GetAllTopicsDes();
    } else {
      getAllTrainingTopic();
    }
  }, [designation,trainingType]);
  ...
  const addSingleTraining = () => {
    ...
    if (!selectedOption || data.length <= 0 || trainingDate === "" ) {
      ...
      const newTrainings = [];
console.log(selectedOption,"addSingleTraining")
console.log(selectedNameOption,"addSinglenaemTraining")
...
}, [designation, selectedNameOption]);
async function updateTrainingTopics() {
  // Set the selected name option
  //await setSelectedNameOption(newEmpNames);

  // Optionally update departments if needed
  // await setDepartments(newDepartments);

  // Check the training type and call the appropriate function
  if (trainingType === "5b650378-6fd6-4e92-b182-cc48ad58e6ad" && !code.includes('select_all') ) {
    //await GetAllTopicsDes();
    await GetAllTopicsDesDept();
    //await getAllTrainingTopic();
  }
...
// Call the function to update training topics
  ...
  const GetAllTopicsDesDept = () => {
    // Ensure designation is defined and not empty
    if (!designation || designation.length === 0 || !designation[0].value) {
        console.error("designation is not properly defined or is empty.");
        return;
    }

    console.log(designation.value, "designation 393 GetAllTopicsDesDept");
    const selectedValues = designation.map((item) => item.value);
    const commaSeparatedValues = selectedValues.join(',');
    console.log(commaSeparatedValues,"GetAllTopicsDesDeptmethod");
    //const url = `${UrlData}CompentencyMaster/GetAllTopicsDes?designation=${commaSeparatedValues}`;
    axios({
        method: "get",
        url: `${UrlData}CompentencyMaster/GetAllTopicsDes?designation=${commaSeparatedValues}`,
    })
    .then((response) => {
        console.log(response, "get all topics des");

        const trainingTopics = response.data.data.map((item) => ({
            value: item.t_id,
            label: item.t_description,
        }));
