...
const AddTrainingFeedback = () => {
  ...
  const [trainingRel, setTrainingRel] = useState("");
  const [question, setQuestion] = useState("");
  const [duration, setDuration] = useState("");
  const [example, setExample] = useState("");
  const [contentWell, setContentWell] = useState("");
  ...
  const getAllScoreId = () => {
    axios
      .get(
        new URL(
          UrlData +
          `ParameterValueMaster/GetAll?Parameterid=9bf6d121-d33d-4aef-8660-5728f1715d50&status=1`
        )
      )
      .then((response) => {
        console.log("get all training nature", response.data.data);
        setAllScore(response.data.data);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const addSubFeedback = () => {
    ...
    if (
      ...
      const newFeedback = {
        ...
        f_trainingRel: trainingRel,
        f_contentWell: contentWell,
        f_example: example,
        f_duration: duration,
  ...
  const getSubFeedback = (index) => {
    ...
    setTrainingRel(feedbackToEdit.f_trainingRel);
    setQuestion(feedbackToEdit.f_question);
    setDuration(feedbackToEdit.f_duration);
    setExample(feedbackToEdit.f_example);
    setContentWell(feedbackToEdit.f_contentWell);
    setParticipation(feedbackToEdit.f_participation);
                      ...
                      >
                        <thead>
                          <tr>
                            ...
                            <th scope="col" style={headerCellStyle}>
                              Training Was relevant & i will be able to apply in my work
                            </th>
                            <th scope="col" style={headerCellStyle}>
                            ...
                            <th scope="col" style={headerCellStyle}>
                              Participation & Interaction session
                            </th>
                            <th scope="col" style={headerCellStyle}>
                            ...
                            currentItems.map((data, index) => {
                              ...
                              return (
                                <tr key={adjustedIndex}>
                                  ...
                                  <td>{data.f_trainingRel}</td>
                                  <td>{data.f_contentWell}</td>
                                  <td>{data.f_example}</td>
                                  <td>{data.f_duration}</td>
                                  <td>{data.f_participation}</td>
                                  <td>{data.f_question}</td>
                                  <td>
                                    <Edit
              ...
              <div className="col-xs-12 col-sm-12 col-md-12 col-lg-6">
                <div className="form-group form-group-sm">
                  ...
                  <select
                    className="form-select"
                    value={trainingRel}
                    onChange={(e) => setTrainingRel(e.target.value)}
                  >
                    <option value="" disabled>
                      Select
                    </option>
                    {allScore.map((data, index) => (
                      <option key={index} value={data.pv_parametervalue}>
                        {data.pv_parametervalue}
                      </option>
                    ))}
              ...
              <div className="col-xs-12 col-sm-12 col-md-12 col-lg-6 mt-lg-0 mt-4">
                <div className="form-group form-group-sm">
                  ...
                  <select
                    className="form-select"
                    value={contentWell}
                    onChange={(e) => setContentWell(e.target.value)}
                  >
                    <option value="" disabled>
                      Select 
                    </option>
