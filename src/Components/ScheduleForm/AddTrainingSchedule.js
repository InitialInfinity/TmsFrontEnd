...
const AddTrainingSchedule = () => {
  ...
  const [trainingRequestedArray, setTrainingRequestedByArray] = useState([]);
  ...
  const getAllData = () => {
    axios
      ...
      .then((response) => {
        ...
        setTrainingRequestedByArray(response.data.data);
  ...
  const addSingleHrTraining = () => {
    ...
    // }
    if (
      (trainingAttended === "Yes" || trainingAttended === null) &&
      (!scheduledHours || !actualHoursAttended)
    ) {
      alert("Please fill all the details");
    }
  ...
  const isTrainingAlreadyAdded = () => {
    // Check if the training with the specific tss_id already exists in the allSubTrainingSchedule array
    return allSubTrainingSchedule.some(
                  ...
                  <div className="col-xs-12 col-sm-12 col-md-12 col-lg-4">
                    <div className="form-group form-group-sm">
                      ...
                      >
                        ...
                        {trainingRequestedArray.map((data, index) => (
                          <option key={index} value={data.emp_code}>
                            {data.emp_fname}
                          </option>
                        ))}
                      </select>
            ...
            {action1 === "Approved By Manager" ? (
              <>
                <Edit
                  ...
                  type="button"
                  onClick={() => {
                    getSingleHrTraining(adjustedIndex, data.tss_id);
                    FirstHandleShow();
                  }}
                />
                <Edit
                  ...
                  type="button"
                  onClick={() => {
                    getSingleHodTraining(adjustedIndex, data.tss_id);
                    SecondHandleShow();
                  }}
      ...
      >
        Save
            ...
            {/* <Button
              style={{ backgroundColor: "#1B5A90" }}
              onClick={
                isTrainingAlreadyAdded()
                  ? updateSingleHrTraining
                  : addSingleHrTraining
              }
            >
              {isTrainingAlreadyAdded() ? "Update" : "Save"}
            </Button> */}
